...
const createAudio = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    return respondsSender(
      null,
      "Please fill in all fields: userId, translatedWord, languageTranslatedTo, recordedVoice, wordId",
...
const getAudios = asyncHandler(async (req, res) => {
  ...
  respondsSender(
    Audios,
    "All audio files gotten successfully",
    ResponseCode.successful,
    res
  );
});
...
const updateAudio = asyncHandler(async (req, res) => {
  const { userId, translatedWord, languageTranslatedTo, recordedVoice } =
    req.body;
  // validation
