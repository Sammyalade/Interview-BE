...
const protect = (allowedRole) => {
    return asyncHandler(async (req, res, next) => {
        ...
        try {
            ...
            jwt.verify(bearerToken, process.env.JWT_SECRET, async (err, decodedToken) => {
                if (err) {
                    ...
                    try {
                        ...
                        if (!user || user.role !== allowedRole) {
                            return respondsSender(null, "You are not allowed here", ResponseCode.noData, res);
                        }

                        next();
                    } catch (error) {
                        return respondsSender(null, `Error: ${error}`, ResponseCode.noData, res);
                    }
                }
            });

        } catch (error) {
            return respondsSender(null, "Not authorized, Please login" + error, ResponseCode.unAuthorized, res);
        }
    });
};
