    ...
    .map((dialogue) => {
      ...
      for (const key in dialogue) {
        if (Object.hasOwnProperty.call(dialogue, key)) {
          cleanedDialogue[key] = dialogue[key].filter(
            (line) => line.trim() !== ""
          );
        }
      }
  ...
  try {
    ...
    for (const item of cleanedDialogues) {
      ...
      for (const dialogueText of dialogueTexts) {
        ...
        if (words.length < 2) {
          console.error(
            `Invalid dialogue format: '${dialogueText}'. Skipping...`
          );
          continue;
        }

        const subDialogueText = words[1].trim();
        const existingSubDialogue = await subDialogue.findOne({
...
const fetchDialogues = asyncHandler(async (req, res) => {
  try {
    ...
    respondsSender(
      "Dialogues added to the database successfully",
      null,
